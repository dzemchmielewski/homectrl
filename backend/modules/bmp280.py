...
# Ported to Python: dzemchmielewski

# Power Modes
BMP280_POWER_SLEEP = 0
BMP280_POWER_FORCED = 1
...
BMP280_STANDBY_0_5 = 0
BMP280_STANDBY_62_5 = 1
...
BMP280_OS_ULTRALOW = 0
BMP280_OS_LOW = 1
BMP280_OS_STANDARD = 2
BMP280_OS_HIGH = 3
BMP280_OS_ULTRAHIGH = 4
...
# Use cases
BMP280_CASE_HANDHELD_LOW = 0
BMP280_CASE_HANDHELD_DYN = 1
BMP280_CASE_WEATHER = 2
BMP280_CASE_FLOOR = 3
BMP280_CASE_DROP = 4
BMP280_CASE_INDOOR = 5
...
_BMP280_REGISTER_ID = 0xD0
_BMP280_REGISTER_RESET = 0xE0
_BMP280_REGISTER_STATUS = 0xF3
_BMP280_REGISTER_CONTROL = 0xF4
_BMP280_REGISTER_CONFIG = 0xF5  # IIR filter config
...
class BMP280:
    ...
    def _read(self, addr, size=1):
        return bytes(self._bmp_i2c.read_i2c_block_data(self._i2c_addr, addr, size))

    def _write(self, addr, b_arr):
        if not type(b_arr) is bytearray:
            b_arr = bytearray([b_arr])
        self._bmp_i2c.write_i2c_block_data(self._i2c_addr, addr, list(b_arr))
    ...
    def use_case(self, uc):
        ...
        self._write(_BMP280_REGISTER_CONFIG, bytearray([(iir << 2) + (sb << 5)]))
        self._write(_BMP280_REGISTER_CONTROL, bytearray([pm + (p_os << 2) + (t_os << 5)]))
