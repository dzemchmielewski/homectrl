...
class WavPlayer:
    ...
    def parse(self, wav_file):
        chunk_ID = wav_file.index(4)
        if chunk_ID != b"RIFF":
            raise ValueError("WAV chunk ID invalid")
        chunk_size = wav_file.index(4)
        format = wav_file.index(4)
        if format != b"WAVE":
            raise ValueError("WAV format invalid")
        sub_chunk1_ID = wav_file.index(4)
        if sub_chunk1_ID != b"fmt ":
            raise ValueError("WAV sub chunk 1 ID invalid")
        sub_chunk1_size = wav_file.index(4)
        audio_format = struct.unpack("<H", wav_file.index(2))[0]
        num_channels = struct.unpack("<H", wav_file.index(2))[0]
        ...
        self.sample_rate = struct.unpack("<I", wav_file.index(4))[0]
        byte_rate = struct.unpack("<I", wav_file.index(4))[0]
        block_align = struct.unpack("<H", wav_file.index(2))[0]
        self.bits_per_sample = struct.unpack("<H", wav_file.index(2))[0]
        ...
        binary_block = wav_file.index(200)
...
class OneWavPlayer(WavPlayer):
    def __init__(self, id, sck_pin, ws_pin, sd_pin, ibuf, wav_file, enable_pin: Pin = None):
        super().__init__(id, sck_pin, ws_pin, sd_pin, ibuf, "")
        if os.listdir(self.root).count(wav_file) == 0:
            raise ValueError("%s: not found" % wav_file)
    ...
    def play(self, wav_file=None, loop=False):
        if wav_file is not None:
            raise ValueError("OneWavPlayer can only play the WAV file specified at initialization")
        if self.state == WavPlayer.PLAY:
            raise ValueError("already playing a WAV file")
        elif self.state == WavPlayer.PAUSE:
            raise ValueError("paused while playing a WAV file")
