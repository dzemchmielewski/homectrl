...
class BuzzPlayer:
    ...
    async def play_tune(self, tune: RTTTL):
    ...
    def play_song(self, song: str):
        try:
            asyncio.run(self.play_tune(RTTTL(self.find(song))))
        finally:
            self.buzzer.duty(0)

    async def play_song_async(self, song: str):
        try:
            await self.play_tune(RTTTL(self.find(song)))
        finally:
            self.buzzer.duty(0)
